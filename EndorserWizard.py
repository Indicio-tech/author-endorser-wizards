...
async def signTxn(poolHandle, endorserDid, tAA):
    ...
    input(
        """The author will have sent you a Transaction in the file """
        + fileName
        + """. Copy that file to the directory you ran the program from, then press enter."""
    )
...
Press Enter to continue"""
    ...
    while error:
        ...
        if authorTxnReq["endorser"] != endorserDid:
            input(
                "The file sent does not conatain the correct Endorser DID.  Please contact the Author and Request them to input the correct DID in their transaction, then replace the currant Transaction file and press enter."
            )
            error = True
            continue
        else:
    ...
    endorsedTxn = await ledger.multi_sign_request(
        walletHandle, endorserDid, authorTxnReq
    )
...
async def createWallet():
    ...
    walletName = (
        "endorser_wizard_wallet"  # input("What would you like to name your wallet?: ")
    )
    ...
    print(
        "The Wizard will create a wallet for you named '"
        + walletName
        + "'. The key (password) to this wallet is'"
        + walletKey
        + "'"
    )
...
async def openWallet():
    ...
    walletIndex = input(
        "Please select a wallet or hit enter to create a new one("
...
def becomeEndorser():
    print(
        "Please go to https://selfserve.indiciotech.io to become an endorser if you are not already."
    )
...
async def writeAuthorToLedger(poolHandle, authorTxnJson, endorserDid, tAA):
    ...
    if resp["result"]["seqNo"] == None:
        result = ""
        try:
            reqJson = await ledger.build_nym_request(
                endorserDid, authorDid, authorVerKey, None, None
            )
        except:
            print("Error initiating nym request")
        try:
            reqJson = await ledger.append_txn_author_agreement_acceptance_to_request(
                reqJson,
                tAA["data"]["text"],
                tAA["data"]["version"],
                None,
        ...
        try:
            result = await ledger.sign_and_submit_request(
                poolHandle, walletHandle, endorserDid, reqJson
            )
        ...
        if result["op"] == "REJECT":
            print("Result:", result, "\n")
            print(
                "Check the above result to identify why writing the authors did to the network was rejected"
            )
            exit()
        else:
...
async def endorserWizard(endorser):
    ...
    print("\nEndorser Wizard\n-------------\n")
    print(
        'To begin, you must select or add the network that the issuer would like to use for issuing credentials. If you select "Add New Network" you will be given a choice of which network to add to your list  of choices, then that network will be used during the rest  of this session.'
    )
    print()
    ...
    choice = input(
        "The Transaction Endorser Wizard has finished.  Press 'q' to quit, or press enter to go to the main menu: "
    )
...
async def main():
    ...
    endorsedTxn = ""
    signWizard = input(
        """Welcome to the Transaction Endorser Wizard!\n\n
...
referring to the main menu. (Hit 'enter' now to use the wizard, or type 'm' to go to the main menu): """
    ...
    os.system("clear")
    print(
        "Please choose the wallet that contains your Endorser DID on the network the author wants to write a transaction to.  If you do not have one yet you may select 'Create new wallet'.\n"
    )
    await openWallet()
    if signWizard == "m":
        network = ""
    ...
    while endorser:
        ...
        if endorserAction == "q":
            ...
            if poolHandle == 0:
                print(
                    "There is no network connected. Please connect a network first (option 3)"
                )
            elif endorserDid == "":
                print(
                ...
                print(
                    "agreement to the TAA is required for this action. Please agree to the TAA (option 8)"
                )
            else:
                endorsedTxn, endorsedTxnName = await signTxn(
                    poolHandle, endorserDid, tAA
                )
                print("\n")
                print(
                    "Signed txn file:",
            ...
            poolName = str(poolList[int(endorserPool) - 1].values()).replace(
                "dict_values([", ""
            )
            ...
            if poolHandle == 0:
                ...
                authorInfo = {"author_did": authorDid, "author_ver_key": authorVerKey}
