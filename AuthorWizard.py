...
async def createPool(network):
    ...
    config = {"genesis_txn": "genesisFile"}
...
async def createWallet():
    ...
    walletID = {"id": walletName}

    walletKeyJson = {"key": walletKey}
...
def listWallets():
    print("\nWallet Selection\n----------------\n")
    print(
        "Below is a list of the wallets you have available.  Choose the wallet with the Author DID you are going to use to issue credentials."
    )
...
async def openWallet():
    ...
    while error:
        try:
            walletIndex = input(
                "Select the wallet you want to use to issue credentials ("
                + str(len(walletList) + 1)
                + "): "
            )
            print()
            if walletIndex == "":
    ...
    if int(walletIndex) < len(walletList) + 1:
        ...
        print(
            "The index selected exceeds the number of wallets you have.  Selecting default 'Create new wallet'"
        )
    ...
    walletNameConfig = {"id": walletName}
    walletKeyConfig = {"key": walletKey}
...
async def authorWizard(author):
    ...
    print("\nAuthor Wizard\n-------------\n")
    print(
        'To begin, you must select or add the network that you would like to use for issuing credentials. If you select "Add New Network" you will be given a choice of which network to add to your list of choices, then that network will be used during the rest of this session.'
    )
    print()
    poolList = await listPools(role)
    userPool = input(
        "Select the network you want to use(" + str(len(poolList) + 1) + "): "
    )
    print("\n")
    ...
    schemaChoice = input(
        """The first step to creating a credential is to create a new schema or select an existing one.  A schema contains a list of attributes or fields that are to be associated with the credential that you will be issuing.
...
Select an option(1/2): """
    ...
    choice = input(
        "The Transaction Author Wizard has finished.  Press 'q' to quit, or press enter to go to the main menu: "
    )
...
async def createDid(role, walletHandle):
    ...
    print(
        ...
        + role
        + " DID to any other wallet. This seed should be stored in a safe place."
    )
    print()
    while not valid:
        seed = input(
    ...
    try:
        ...
        print()
        input(
            "After you have saved your Seed in a safe place and recorded your "
            + role
            + " DID and Verkey for later use, please hit enter to continue."
        )
        print()
    ...
    try:
        await did.set_did_metadata(
            walletHandle, authorDid[0], json.dumps({"comment": didMetadata})
        )
    except IndyError:
        print("\n")
...
async def listDids(role, walletHandle):
    ...
    print(
        ...
        + ' DID".  Select your '
        + role
        + " DID from the following list, or create a new one and save the seed in a safe place. "
    )
    print()
    try:
        ...
        print("\n")
        print(
            "Error getting the list of dids from current wallet.  Please make sure you have a wallet open."
        )
        print("\n")
    ...
    while error:
        try:
            index = input(
                "Select an " + role + " DID (" + str(len(didList) + 1) + "): "
            )
            print()
            if index == "":
...
async def signSendTxn(authorDid, authorVerKey, authorsTxn, tAA, poolHandle):
    ...
    try:
        authorsTxn = await ledger.append_txn_author_agreement_acceptance_to_request(
            authorsTxn,
            tAA["data"]["text"],
            tAA["data"]["version"],
            None,
    ...
    endorserDid = input(
        "An Endorser must now sign your transaction before you can write it to the network.  Please enter the Endorser DID of your chosen Endorser: "
    )
    try:
    ...
    print(
        "We have completed the first phase of preparing your transaction!\n  The transaction will be in a file named '"
        + fileName
        + "' in the same directory that you started this program from.\n  "
    )
    print()
...
Press Enter to continue\n"""
    ...
    input(
        ...
        + fileName
        + " to your endorser so that they can sign and return it to you, then press enter to continue."
    )

    input(
        """The Endorser will have sent a file named '"""
        + signedFileName
        + """'.
Copy that file to the directory that you started the program from then press enter."""
    ...
    while error:
        try:
            ...
            print(
                "The file does not exist, Please ensure that the endorser sent you the correct file and it is in the directory you ran the program from.\n"
            )
...
async def createSchema(authorDid):
    ...
    print(
        "Schema creation requires you to select a name, a version number, and the attributes you would like to have for the schema. The 'Schema Name' is your choice but should be descriptive enough for you to remember its purpose.\n"
    )
    ...
    print(
        "Enter the schema's attributes names one at a time ('done' to stop, 'restart' to start over).\n"
    )
    while add:
    ...
    error = False
    try:
        schemaId, schema = await anoncreds.issuer_create_schema(
            authorDid, name, version, json.dumps(attrs)
        )
    except:
        error = True
        print("\n")
    ...
    if not error:
        print()
        print(
            "Please record your new Schema ID.  It will be used later when you create credential definitions.\n\n"
        )
...
async def createCredDef(authorDid, poolHandle):
    ...
    print(
        "Credential Definitions, or Cred Defs, only require an existing Schema ID to be created, and allow you to issue credentials containing the specified Schema's attributes"
    )
    print()
    invalidSchema = True
    credDefTxn = '{"none":"0"}'
    while invalidSchema:
        ...
        getSchemaRequest = ""
        try:
            getSchemaRequest = await ledger.build_get_schema_request(
                authorDid, schemaID
            )
        ...
        try:
            ...
            print(
                "Most likely the schema is not on the network, Please try again with a valid schema ID"
            )
            invalidSchema = True
            continue
...
async def transactionAuthorAgreement(poolHandle, walletHandle, authorDid):
    ...
    while not answered:
        ...
        if agreeTAA == "y" or agreeTAA == "Y":
            try:
                add_taa_req = await ledger.build_get_txn_author_agreement_request(
                    authorDid, None
                )
            except IndyError:
                print(
                    "Error appending TAA to your transaction, has a transaction been created?"
                )
            print()
            add_taa_resp = await ledger.sign_and_submit_request(
                poolHandle, walletHandle, authorDid, add_taa_req
            )
            answered = True
            print("You have agreed to the TAA for this session")
        elif agreeTAA == "n" or agreeTAA == "N":
            print(
                "We are sorry, you are not able to write schemas or cred defs to the ledger until you agree to the TAA."
            )
            print()
...
async def main():
    ...
    tAA = ""
    setup = input(
        """Welcome to the Transaction Author wizard!
    ...
    while author:
        ...
        if authorAction == "q":
            ...
            if poolHandle == 0:
                ...
                print(
                    "A transaction has not been created yet. Please create a transaction first (options 1 or 2)"
                )
            elif authorDid == "none" or authorVerKey == "none":
                print(
                    "the Author's DID has not been selected.  Please select a DID to use (option 9)"
                )
            else:
